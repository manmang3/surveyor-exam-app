...
function generateQuestionId(yearStr, questionNum) {
  return `${yearStr}_${questionNum.toString().padStart(2, '0')}`;
}

// 年度文字列を数値に変換する関数
function convertYearToNumber(yearStr) {
  if (yearStr.startsWith('h')) {
    const heisei = parseInt(yearStr.substring(1));
    return 1988 + heisei; // 平成元年は1989年
  } else if (yearStr.startsWith('r')) {
    const reiwa = parseInt(yearStr.substring(1));
    return 2018 + reiwa; // 令和元年は2019年
  }
  return null;
}
  ...
  content = content.replace(
    ...
    (match, offset) => {
      ...
      if (idMatch && idMatch.length > 0) {
        ...
        const questionId = lastIdMatch.match(/"id":\s*"([^"]+)"/)[1];
        
        // 年度と問題番号を抽出してマッピング用のIDを作成
        const parts = questionId.split('_');
        if (parts.length >= 2) {
          const yearStr = parts[0];
          const questionNum = parseInt(parts[1]);
          const mappingId = `${yearStr}-${questionNum}`;
          
          const subcategory = getSubcategory(mappingId);
          return `"category": "不動産登記法",\n    "subcategory": "${subcategory}",`;
        }
      }
      return match;
    }
